...
export function emptyPartnerServiceRow(): PartnerServiceRow {
    return { id: newPartnerCatalogRowId(), serviceId: "", description: "", price: "" };
}

export function emptyPartnerCatalogBlock(initialCategoryId: string): PartnerCategoryBlock {
    return {
...
export function flattenPartnerBlocksForSave(
    ...
    for (const x of meaningful) {
        if (!x.categoryId || !x.serviceId) {
            return { ok: false, message: "Each filled row needs a category and a service (check every block)." };
        }
    }

    if (meaningful.length === 0) {
        return { ok: false, message: "Add at least one category with a service, description, and price." };
    }
