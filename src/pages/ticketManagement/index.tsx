...
const TicketManagement = () => {
  ...
  const orderUnreadThreads = normalChatConversations.filter(
    (c) => c.unreadCount > 0
  ).length;
  const disputeUnreadCount = ticketList.filter(
    (ticket: any) =>
      Number(ticket?.status) === 1 && Number(ticket?.resolve_status) === 1
  ).length;
  const quoteAllCount = quoteChatConversations.length;
  const quoteUnreadThreads = quoteChatConversations.filter(
    (c) => c.unreadCount > 0
  ).length;
  const groupAllCount = groupChatConversations.length;
  const groupUnreadThreads = groupChatConversations.filter(
    (c) => c.unreadCount > 0
  ).length;
  ...
  const chatCards: {
  ...
  }[] = [
    {
      id: "normal",
      title: "General Chats",
      data: { All: orderAllCount, Unread: orderUnreadThreads },
    },
    {
      id: "dispute",
      title: "Dispute Chats",
      data: { Open: disputeUnreadCount, Pending: 0, Closed: 0 },
    },
    {
      id: "quote",
      title: "Quote Chats",
    ...
    },
    {
      id: "group",
      title: "Group Chats",
      data: { All: groupAllCount, Unread: groupUnreadThreads },
    },
  ];
    ...
    () => [
      ...
      },
      {
        ...
        Cell: textUnderlineCell("unique_id", (row) => {
          TicketDetailsDialog.show(row._id, () => refreshData());
        }),
      },
      {
        Header: "Query",
  ...
  return (
    <>
      <div className="main-page-content">
        <CustomHeader title="Ticket Management" />
