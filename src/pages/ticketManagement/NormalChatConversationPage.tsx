...
const getOrderSenderProfile = (sender: NormalChatMessageSender, participants: NormalChatParticipants) => {
    const participant =
        sender === "admin"
            ? participants.admin
            : sender === "employee"
...
const NormalChatConversationPage = () => {
    ...
    const selectedChat =
        normalChatConversations.find((chat) => chat.id === chatId) || normalChatConversations[0];
    const fallbackDetail = {
        chatId: selectedChat.id,
        threadKind: selectedChat.threadKind,
        user: {
    ...
    const detail = normalChatDetails[selectedChat.id] || fallbackDetail;
    const isOrderThread = detail.threadKind === "order";
    ...
    const handleSend = () => {
        ...
        const outgoingAttachments = selectedFiles.map((item) => ({
            id: item.id,
        ...
        const outgoingText =
            text ||
            (outgoingAttachments.length > 0 ? "Shared attachments" : "");
        ...
        <div className="main-page-content">
            <div className="d-flex justify-content-between align-items-center mb-3">
                <div className="d-flex align-items-center gap-2">
                    <button
                        ...
                        aria-label="Back to order chat list"
                    >
                        <i className="bi bi-chevron-left" />
                    </button>
                    <h4 className="m-0 p-0">{isOrderThread ? "Order Chat" : "General Chat"}</h4>
                </div>
            ...
            <div className="row g-3">
                    ...
                    <div className="border rounded-3 bg-white">
                        <div className="d-flex align-items-center justify-content-between p-3 border-bottom">
                            <div className="d-flex align-items-center gap-2">
                                ...
                                >
                                    {isOrderThread ? "O" : selectedChat.userName.charAt(0)}
                                </div>
                                <div>
                                    <h6 className="normal-chat-user-name mb-0">
                                        {isOrderThread ? `Order · ${selectedChat.userId}` : selectedChat.userName}
                                    </h6>
                                    <small className="normal-chat-time">
                                        {isOrderThread
                                            ? `${selectedChat.userName} · Group chat`
                                            : selectedChat.userId}
                                    </small>
                                </div>
                            </div>
                        ...
                        <div className="normal-chat-message-area" ref={messageAreaRef}>
                            {messages.map((msg) => {
                                ...
                                if (isOrderThread && participants) {
                                    const isLeft = msg.sender !== "employee";
                                    const senderProfile = getOrderSenderProfile(msg.sender, participants);
                                    return (
                                        <div
                                            key={msg.id}
                ...
                <div className="col-lg-4">
                    <button type="button" className="btn custom-btn-primary mb-3">
                        Transfer Chat
                    </button>
                    <div className="border rounded-3 bg-white p-3">
                        ...
                        {isOrderThread && participants && (
                            <>
                                <h6 className="normal-chat-section-title">Participants</h6>
                                <div className="small mb-3">
                                    <div className="mb-1">
                                        <span className="text-muted">Admin: </span>
                                        <span className="fw-semibold">{participants.admin.name}</span>
                                    </div>
                                    <div className="mb-1">
                                        <span className="text-muted">Employee: </span>
                                        <span className="fw-semibold">{participants.employee.name}</span>
                                    </div>
                                    <div className="mb-1">
                                        <span className="text-muted">Partner: </span>
                                        <span className="fw-semibold">{participants.partner.name}</span>
                                    </div>
                                    <div className="mb-1">
                                        <span className="text-muted">User: </span>
                                        <span className="fw-semibold">{participants.user.name}</span>
                                    </div>
                                </div>
                            </>
                        )}
...
export default NormalChatConversationPage;
