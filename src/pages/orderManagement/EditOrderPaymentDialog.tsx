...
const EditOrderPaymentDialog: React.FC<EditOrderPaymentDialogProps> & {
  ...
  const {
    ...
    defaultValues: {
      ...
      payment_mode_id: orderDetails.payment_mode_id
  ...
  const [totalPriceState, setTotalPriceState] =
    useState<number>(defaultTotalPrice);
  const [paidTotalState, setPaidTotalState] =
    useState<number>(defaultPaidTotal);
  ...
  const [orderDateUserState, setOrderDateUserState] = useState<string | null>(
    orderDetails.order_date ?? null
  );
  const [orderDatePartnerState, setOrderDatePartnerState] = useState<
    string | null
  >(orderDetails.order_date ?? null);

  const balanceTotalState = useMemo(
    () => Math.max(0, totalPriceState - paidTotalState),
    [totalPriceState, paidTotalState]
  );
  const balanceSubState = useMemo(
    () => Math.max(0, subTotalState - paidSubState),
    [subTotalState, paidSubState]
  );
  ...
  const onSubmitEvent = async (data: EditOrderPaymentForm) => {
    ...
    const normalizedPaidTotal =
      paidTotalNum >= totalPriceNum ? totalPriceNum : 0;
  ...
  return (
    ...
    >
      ...
      <Modal.Body className="px-4 pb-4 pt-0">
        <form
          noValidate
          name="edit-order-payment-form"
          id="edit-order-payment-form"
