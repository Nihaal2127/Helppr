...
const EditOrderServiceDialog: React.FC<EditOrderServiceDialogProps> & {
  ...
  const {
    ...
    defaultValues: {
      service_date: orderItemModel.service_date
        ? orderItemModel.service_date
        : "",
      service_from_time: orderItemModel.service_from_time
        ? orderItemModel.service_from_time
        : "",
      service_to_time: orderItemModel.service_to_time
        ? orderItemModel.service_to_time
        : "",
      service_status: orderItemModel.service_status
        ? orderItemModel.service_status
        : 1,
  ...
  const statuses: { value: string; label: string }[] = Array.from(
    ...
    .filter(
      ([_, value]) => value.label !== "Cancelled" && value.label !== "Refunded"
    )
    .map(([key, value]) => ({
      value: key.toString(),
      label: value.label,
          ...
          >
            <Row>
              ...
              <CustomTextFieldRadio
                ...
                defaultValue={
                  orderItemModel?.service_status
                    ? String(orderItemModel?.service_status)
                    : "1"
                }
                setValue={setValue}
              />
