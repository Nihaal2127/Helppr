...
import { partnerPortfoliosSeed, partnerPostsSeed, partnerSubscriptionsSeed } from "../mockData/partnerManagementMockData";
...
let mockSubscriptionPlans: SubscriptionPlanModel[] = partnerSubscriptionPlansSeed.map((item) => ({ ...item }));
...
function parseSubscriptionPlanRecord(raw: Record<string, unknown>): SubscriptionPlanModel {
  ...
  return {
    ...
    priority: raw.priority != null && raw.priority !== "" ? String(raw.priority) : "",
    is_active: Boolean(raw.is_active),
  };
}

function pickSubscriptionPlanListRoot(d: Record<string, unknown>): Record<string, unknown> {
  const inner = d.data;
...
function subscriptionPlanListStatsFromResponse(
  ...
  const total = Number(root.total ?? root.totalDocs ?? root.total_count ?? root.count);
  const activeN = Number(root.active_count ?? root.activeCount ?? root.active_total);
  const inactiveN = Number(root.inactive_count ?? root.inactiveCount ?? root.inactive_total);
  if (Number.isFinite(total) && Number.isFinite(activeN) && Number.isFinite(inactiveN)) {
    return { Total: total, Active: activeN, Inactive: inactiveN };
  }
  if (Number.isFinite(total) && Number.isFinite(activeN)) {
    return { Total: total, Active: activeN, Inactive: Math.max(0, total - activeN) };
  }
  return statsFor(records);
}

function buildSubscriptionPlanCreateBody(plan: SubscriptionPlanModel): Record<string, unknown> {
  const body: Record<string, unknown> = {
    plan_name: String(plan.plan_name ?? "").trim().toLowerCase(),
    ...
    duration_type: String(plan.duration_type ?? "").trim().toLowerCase(),
...
function buildSubscriptionPlanUpdateBody(plan: SubscriptionPlanModel): Record<string, unknown> {
  const body: Record<string, unknown> = {
    plan_description: String(plan.plan_description ?? "").trim(),
    price: Number(String(plan.price ?? "").replace(/,/g, "")) || 0,
    duration: Number(String(plan.duration ?? "").replace(/,/g, "")) || 0,
...
export async function fetchSubscriptionPlans(
  ...
  if (USE_MOCK_SUBSCRIPTION_PLAN_API) {
    ...
    return { response: true, records, totalPages, stats: statsFor(mockSubscriptionPlans) };
  }
  ...
  if (filters.status && filters.status !== "all") {
    ...
    else if (s === "inactive" || s === "expired") params.set("is_active", "false");
  ...
  const res = await apiRequest(`${ApiPaths.SUBSCRIPTION_PLAN_GET_ALL()}?${params.toString()}`, "GET");
  if (!res.success) {
    return { response: false, records: [], totalPages: 0, stats: { Total: 0, Active: 0, Inactive: 0 } };
  }

  const root = pickSubscriptionPlanListRoot((res.data ?? {}) as Record<string, unknown>);
  const rawList = (root.records ?? root.list ?? []) as Record<string, unknown>[];
  const records = Array.isArray(rawList) ? rawList.map((r) => parseSubscriptionPlanRecord(r)) : [];
...
export async function voidSubscriptionPlan(id: string): Promise<boolean> {
  if (USE_MOCK_SUBSCRIPTION_PLAN_API) {
    mockSubscriptionPlans = mockSubscriptionPlans.map((p) => (p._id === id ? { ...p, is_active: false } : p));
    return true;
  }
...
export async function saveSubscriptionPlan(plan: SubscriptionPlanModel, isUpdate: boolean): Promise<boolean> {
  if (USE_MOCK_SUBSCRIPTION_PLAN_API) {
    if (isUpdate) {
      mockSubscriptionPlans = mockSubscriptionPlans.map((p) => (p._id === plan._id ? { ...p, ...plan } : p));
      return true;
    }
    const nextId = `PLN${String(Date.now()).slice(-6)}`;
    mockSubscriptionPlans = [{ ...plan, _id: nextId }, ...mockSubscriptionPlans];
    return true;
  }

  if (isUpdate) {
    if (!plan._id) return false;
    const res = await apiRequest(ApiPaths.SUBSCRIPTION_PLAN_UPDATE(plan._id), "PUT", buildSubscriptionPlanUpdateBody(plan));
    return Boolean(res.success);
  }
  const res = await apiRequest(ApiPaths.SUBSCRIPTION_PLAN_CREATE, "POST", buildSubscriptionPlanCreateBody(plan));
  return Boolean(res.success);
}

export async function fetchSubscriptionPlanDropDown(): Promise<{ value: string; label: string }[]> {
  if (USE_MOCK_SUBSCRIPTION_PLAN_API) {
    return partnerSubscriptionPlansSeed
      .filter((p) => p.is_active)
      .map((p) => ({ value: p.plan_name, label: capitalizeString(p.plan_name) }));
  }

  const res = await apiRequest(ApiPaths.SUBSCRIPTION_PLAN_GET_DROP_DOWN(), "GET");
  if (!res.success) return [];

  const root = pickSubscriptionPlanListRoot((res.data ?? {}) as Record<string, unknown>);
  const rawList = (root.records ?? root.list ?? res.data?.records ?? []) as Record<string, unknown>[];
  if (!Array.isArray(rawList)) return [];

  const out: { value: string; label: string }[] = [];
  for (const r of rawList) {
    const name = String(r.plan_name ?? "").trim().toLowerCase();
    if (!name) continue;
    if (r.is_active === false) continue;
    const id = r._id != null && String(r._id).trim() !== "" ? String(r._id) : "";
    out.push({ value: id || name, label: capitalizeString(name) });
  }
  return out;
}

function pickPartnerSubListRoot(d: Record<string, unknown>): Record<string, unknown> {
  const inner = d.data;
  if (inner && typeof inner === "object" && !Array.isArray(inner)) {
    return inner as Record<string, unknown>;
  }
...
function mapPartnerSubscriptionApiRecord(raw: Record<string, unknown>): PartnerSubscriptionModel {
  const id = String(
    raw._id ??
      raw.id ??
      raw.subscription_id ??
  ...
  const start = String(raw.started_at ?? raw.subscription_start_date ?? raw.start_date ?? "").slice(0, 10);
  const end = String(raw.expires_at ?? raw.subscription_end_date ?? raw.end_date ?? "").slice(0, 10);
...
function partnerSubListStatsFromResponse(root: Record<string, unknown>, records: PartnerSubscriptionModel[]): ListStats {
  const st = root.stats as ListStats | undefined;
  if (st && typeof st.Total === "number") return st;
  const total = Number(root.total ?? root.totalDocs ?? root.total_count ?? root.count);
...
export async function fetchPartnerSubscriptions(
  ...
  if (USE_MOCK_PARTNER_SUBSCRIPTIONS_API) {
    ...
    if (statusRaw === "false" || statusRaw === "inactive" || statusRaw === "expired") {
  ...
  if (statusLower && statusLower !== "all") {
    if (statusLower === "active" || statusLower === "true") params.set("status", "active");
    else if (statusLower === "inactive" || statusLower === "false") params.set("status", "cancelled");
  ...
  const res = await apiRequest(`${ApiPaths.PARTNER_SUBSCRIPTION_GET_ALL()}?${params.toString()}`, "GET");
  if (!res.success) return { response: false, records: [], totalPages: 0, stats: { Total: 0, Active: 0, Inactive: 0 } };
  const root = pickPartnerSubListRoot((res.data ?? {}) as Record<string, unknown>);
  const rawList = (root.records ?? root.list ?? []) as Record<string, unknown>[];
  let records = Array.isArray(rawList) ? rawList.map((r) => mapPartnerSubscriptionApiRecord(r)) : [];
  const planTypeSlug = (filters.planType ?? "").trim().toLowerCase();
  if (planTypeSlug && planTypeSlug !== "all" && !/^[a-f\d]{24}$/i.test(planTypeSlug)) {
    records = records.filter((r) => (r.subscription_plan || "").toLowerCase() === planTypeSlug);
  }
  const totalPages = Number(root.totalPages ?? root.total_pages ?? 0) || 0;
...
export async function voidPartnerSubscription(id: string): Promise<boolean> {
  ...
  const res = await apiRequest(ApiPaths.PARTNER_SUBSCRIPTION_DELETE(id), "DELETE");
  return Boolean(res.success);
}
 ...
 */
export async function savePartnerSubscription(sub: PartnerSubscriptionModel): Promise<boolean> {
  ...
  if (updateExisting) {
    ...
    if (sub.subscription_start_date?.trim()) body.started_at = sub.subscription_start_date.trim();
    if (sub.subscription_end_date?.trim()) body.expires_at = sub.subscription_end_date.trim();
    const pid = (sub.subscription_plan_id ?? "").trim();
    if (pid && /^[a-f\d]{24}$/i.test(pid)) {
      body.subscription_plan_id = pid;
    }
    const res = await apiRequest(ApiPaths.PARTNER_SUBSCRIPTION_UPDATE(String(sub._id)), "PUT", body);
    return Boolean(res.success);
  }

  const planId = (sub.subscription_plan_id || sub.subscription_plan || "").trim();
  ...
  if (sub.subscription_start_date?.trim()) createBody.started_at = sub.subscription_start_date.trim();
  if (sub.subscription_end_date?.trim()) createBody.expires_at = sub.subscription_end_date.trim();
  const res = await apiRequest(ApiPaths.PARTNER_SUBSCRIPTION_CREATE, "POST", createBody);
  return Boolean(res.success);
}
...
export function updatePartnerPostStatus(postId: number | undefined, status: PostModel["status"]): void {
